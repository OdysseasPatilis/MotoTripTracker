package com.odys.mototriptracker.data.sensor
...
@Singleton
class AndroidGForceTracker @Inject constructor(
    @ApplicationContext context: Context,
) : GForceTracker, SensorEventListener {

    private val sensorManager = context.getSystemService(Context.SENSOR_SERVICE) as SensorManager
    private val linearAccelSensor: Sensor? =
        sensorManager.getDefaultSensor(Sensor.TYPE_LINEAR_ACCELERATION)

    override var currentGForce: Float = 0f
        private set

    override var maxSessionGForce: Float = 0f
        private set

    /** Horizontal acceleration magnitude (approx lateral / braking plane). */
    override var currentLateralGForce: Float = 0f
        private set

    override var maxSessionLateralGForce: Float = 0f
        private set

    override fun startTracking(resetSession: Boolean) {
        if (resetSession) {
            currentGForce = 0f
            maxSessionGForce = 0f
